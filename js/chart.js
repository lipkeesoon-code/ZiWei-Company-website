...
const UI = {
    phoneContainer: (document.getElementById('phone-container') || document.createElement('div')),
    bottomNav: (document.getElementById('bottom-nav') || document.createElement('div')),
    views: document.querySelectorAll('.app-view'),
    navItems: document.querySelectorAll('.nav-item'),
    
    // Login
    loginEmail: (document.getElementById('login-email') || document.createElement('div')),
    loginPass: (document.getElementById('login-pass') || document.createElement('div')),
    btnLogin: (document.getElementById('btn-login-submit') || document.createElement('div')), // Updated ID
    loginError: (document.getElementById('login-error') || document.createElement('div')),
    portalBtns: (document.getElementById('portal-buttons') || document.createElement('div')),
    loginFormContainer: (document.getElementById('login-form-container') || document.createElement('div')),
    loginFormTitle: (document.getElementById('login-form-title') || document.createElement('div')),
    adminIconContainer: (document.getElementById('admin-icon-container') || document.createElement('div')),

    btnPortalAdmin: (document.getElementById('btn-portal-admin') || document.createElement('div')),
    btnPortalMember: (document.getElementById('btn-portal-member') || document.createElement('div')),
    btnLoginBack: (document.getElementById('btn-login-back') || document.createElement('div')),
    btnLogout: (document.getElementById('btn-logout') || document.createElement('div')),

    // Admin Settings
    approvedMemberList: (document.getElementById('approved-member-list') || document.createElement('div')),
    btnAdminBack: (document.getElementById('btn-admin-back') || document.createElement('div')),
    btnAddMember: (document.getElementById('btn-add-member') || document.createElement('div')),
    
    // Input
    gregYear: (document.getElementById('greg-year') || document.createElement('div')),
    gregMonth: (document.getElementById('greg-month') || document.createElement('div')),
    gregDay: (document.getElementById('greg-day') || document.createElement('div')),
    gregTime: (document.getElementById('greg-time') || document.createElement('div')),
    lunarDisplay: (document.getElementById('lunar-date-display') || document.createElement('div')),
    userName: (document.getElementById('user-name') || document.createElement('div')),
    btnSaveChart: (document.getElementById('btn-save-chart') || document.createElement('div')),
    btnNewChart: (document.getElementById('btn-new-chart') || document.createElement('div')),

    // Records
    groupList: (document.getElementById('group-list') || document.createElement('div')),
    addGroupBtn: (document.getElementById('add-group-btn') || document.createElement('div')),
    recordSearch: (document.getElementById('record-search') || document.createElement('div')),
    
    // Main Board
    mainBoard: (document.getElementById('main-board') || document.createElement('div')),
    cName: (document.getElementById('c-name') || document.createElement('div')),
    cGregorian: (document.getElementById('c-gregorian') || document.createElement('div')),
    cLunar: (document.getElementById('c-lunar') || document.createElement('div')),
    cGender: (document.getElementById('c-gender') || document.createElement('div')),
    cZodiac: (document.getElementById('c-zodiac') || document.createElement('div')),
    cElements: (document.getElementById('c-elements') || document.createElement('div')),
    cAge: (document.getElementById('c-age') || document.createElement('div')),
    cFormation: (document.getElementById('c-formation') || document.createElement('div')),
    infoListMobile: (document.getElementById('info-list-mobile') || document.createElement('div')),
    
    // Global Funcs
    snapshotBtn: (document.getElementById('snapshot-btn') || document.createElement('div')),
    importBtn: (document.getElementById('import-btn') || document.createElement('div')),
    importFileInput: (document.getElementById('import-file-input') || document.createElement('div')),
    bgChangeBtn: (document.getElementById('bg-change-btn') || document.createElement('div')),
    bgUploadInput: (document.getElementById('bg-upload-input') || document.createElement('div')),
    bgLayer: (document.getElementById('bg-layer') || document.createElement('div')),
    
    // Gallery
    btnChangePass: (document.getElementById('change-pass-btn') || document.createElement('div')),
    btnCloudSync: (document.getElementById('btn-cloud-sync') || document.createElement('div'))
};
...
function showLockdownOverlay() {
    const overlay = (document.getElementById('security-lockdown') || document.createElement('div'));
...
function handleAdminPrivileges() {
    if (State.isAdmin) {
        ...
        if (!(document.getElementById('admin-spanner') || document.createElement('div'))) {
...
function hideAdminPrivileges() {
    const spanner = (document.getElementById('admin-spanner') || document.createElement('div'));
...
function getZihuaSVG(type) {
    const COLOR_MAP = { '禄': '#3FB532', '权': '#1C77FC', '科': '#F5A832', '忌': '#E60012' };
    const zhClass = (type === '禄'?'zh-lu':(type === '权'?'zh-quan':(type === '科'?'zh-ke':'zh-ji')));
    const color = COLOR_MAP[type] || '#333';
    return `<svg class="zihua-circle ${zhClass}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="${color}" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" style="width:13px;height:13px;display:inline-block;vertical-align:top;"><path d="M23 4v6h-6"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>`;
}
...
function initDropdowns() {
    for (let y = 1926; y <= 2126; y++) UI.gregYear.add(new Option(y + '年', y));
...
function getLocalCoords(el, board) {
    ...
    return {
        ...
        y: (erect.top - brect.top - 2) * (550 / brect.height) // 飞到头顶上，不盖住字
    };
}
...
function getPalaceCenter(idx) {
    ...
    const board = (document.getElementById('main-board') || document.createElement('div'));
...
function getStemCenter(idx) {
    ...
    const brect = board.getBoundingClientRect();
    const stemEl = box.querySelector('.stem');
    if (stemEl) {
        const erect = stemEl.getBoundingClientRect();
        return {
            x: (erect.left - brect.left + erect.width / 2) * (366 / brect.width),
            y: (erect.top - brect.top + erect.height / 2) * (550 / brect.height)
        };
    }
    
    const rect = box.getBoundingClientRect();
    return {
        x: (rect.left - brect.left + 15) * (366 / brect.width),
        y: (rect.bottom - brect.top - 15) * (550 / brect.height)
    };
}
...
function renderMainBoard(record) {
    ...
    palaces.forEach((p, idx) => {
        ...
        // Interaction logic for selection and flying lines has been removed per user request
    ...
    if (centerArea) {
        if (!centerArea.querySelector('.brand-signature')) {
            const logoSrc = typeof LOGO_BASE64 !== 'undefined' ? LOGO_BASE64 : '设计风格/5cm W .png';
            centerArea.insertAdjacentHTML('beforeend', `<img src="${logoSrc}" class="brand-signature" alt="Brand Logo">`);
        }
        if (!centerArea.querySelector('.grey-bg-logo')) {
            centerArea.insertAdjacentHTML('beforeend', `<img src="设计风格/Grey ZiFu Logo 001 .png" class="grey-bg-logo" alt="Grey Background Logo">`);
        }
    }
...
function performSnapshot() {
    ...
    const flashEl = (document.getElementById('camera-flash') || document.createElement('div'));
    if (flashEl) {
    ...
    if (!(document.getElementById('view-main') || document.createElement('div')).classList.contains('active')) {
        // switchView('view-main');
    }

    const target = (document.getElementById('main-board') || document.createElement('div'));
...
document.addEventListener('click', () => {
    (document.getElementById('context-menu') || document.createElement('div')).style.display = 'none';
});

(document.getElementById('menu-rename') || document.createElement('div')).addEventListener('click', () => {
...
(document.getElementById('menu-delete') || document.createElement('div')).addEventListener('click', () => {
    const { type, id } = contextTarget;
